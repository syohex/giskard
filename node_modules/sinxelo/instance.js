/*jslint node:true, devel: true, plusplus: true, vars: true */
/*global config */

'use strict';

var util = require('util');
var events = require('events');
var http = require('http');
var url = require('url');
var path = require('path');
var fs = require('fs');
var domains = require("domain");
var mime = require('./mime');
var FileResolver = require('./FileResolver');

var server = {

	server: {
		value: http.createServer()
	},

	root: {
		value: ''
	},
    
    showFileContent: {
        enumerable: false,
        value: function (res, file, context, code) {
            
            /*
            fileResolver.on('read', function (event) {

                var ext = path.extname(file).replace('.', '');
                var type = mime.type(ext);
                
                if (type) {
                    res.setHeader("Content-Type", type);
                }

                res.statusCode = code;

                res.write(event.data);
                res.end();
            });
            
            console.log(file);
            fileResolver.resolve(file);
            */
            res.statusCode = code;
            
            fs.createReadStream(file).pipe(res);
        }
    },

	handleError: {
		enumerable: false,
		value: function (res, code, error) {
            
            var self = this;
            var file;
			
            if (code === 404) {
                file = require.resolve('sinxelo/404.html');
                res.statusCode = 404;
                //self.showFileContent(res, require.resolve('sinxelo/404.html'), self, code);
            } else if (code === 500) {
                file = require.resolve('sinxelo/500.html');
                res.statusCode = 500;
                //self.showFileContent(res, require.resolve('sinxelo/500.html'), self, code);
            }
            
            if (!file) {
                res.write(error.message);
                res.end();
            } else {
                var resolver = new FileResolver(file, function(){
                    console.log("Responded with " + code);
                    res.end();
                }, function(err){
                    console.log(err)
                }, res);
                
                resolver.resolve();   
            }
            
            
            
            return false;
		}
	},

	handleRequest: {
		enumerable: false,
		value: function (req, res, context) {
            
            var self = this,
                urlData = url.parse(req.url, true),
                href = urlData.href,
                query = urlData.query,
                file = path.join(this.basePath + (href && href !== '/' ? href : '/index.html'));
            
            console.log("New request " + req.url);
            /*
            fileResolver.on('error', function (event) {
                self.handleError(res, event.code, event.message);
            });
            
            fileResolver.on('read', function (event) {

                var ext = path.extname(file).replace('.', '');
                var type = mime.type(ext);
                
                if (type) {
                    
                    /*
                    if (type === 'text/html') {
                        var content = event.data.toString("utf-8");
                        event.data = self.loadTemplates(content, context);
                    }
                    */
                    /*
                    res.setHeader("Content-Type", type);
                }

                res.statusCode = 200;

                res.write(event.data);
                res.end();
            });
            
            fileResolver.resolve(file);
            */
            
            
            var ext = path.extname(file).replace('.', '');
            var type = mime.type(ext);
            
            if (type) {
                res.setHeader("Content-Type", type);
            }
    
            var resolver = new FileResolver(file, function(){
                console.log("Responded to " + req.url + " with 200");
                res.end();
            }, function(err){
                self.handleError(res, err.code, err.message);
            }, res);
            
            resolver.resolve();
            
//            var fileStream = fs.createReadStream(file);
//            
//            
//            fileStream.pipe(res);
//            fileStream.on('end', function () {
//                res.end();
//            });
		}
	},

	start: {
		value: function (root, port) {

			var self = this;
            
			this.basePath = root;

			this.server.on('request', function onRequest(req, res) {

				var domain = domains.create();
				
				domain.on('error', function (err) {
                    console.log("An uncaught error happened " + err.code + "::" + err.path);
					self.handleError(res, 500, err);
					domain.dispose();
				});

				domain.enter();
				self.handleRequest(req, res, self);
			});

			this.server.listen(port || 80);
            
            console.log("Started to listen to port " + (port || 80));
		}
	}
};

// Create the instance
var instance = Object.create(events.EventEmitter.prototype, server);

// Get the config object
var config = JSON.parse(process.env.config);

// Initialize the instance
instance.start(config.path, config.port);

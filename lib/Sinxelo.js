...
var Sinxelo = function(config) {
     ...
     * @type {string}
     */
    this._configPath = config || Sinxelo.DEFAULT_CONFIG;
     ...
     * @type {number}
     */
    this._workers = new Array(os.cpus());
...
/**
 * Apply the configuration fro the file or throw an exception if the file
...
Sinxelo.prototype._applyConfig = function(callback, err, data) {
    ...
    // Posible file errors
    if (err) {
        this._logger.error('Cannot load the config file (' + err.message + ')');
        return false;
    }
...
Sinxelo.prototype._startWorkers = function() {
    ...
    for (i = 0; i < this._workers.length; i++) {
        ...
        worker = cluster.fork({config: JSON.stringify(this._config), session: this._global});
 ...
 * @conts
 */
Sinxelo.DEFAULT_CONFIG = process.cwd() + '/config.json';

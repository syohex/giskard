...
process.on('uncaughtException', function (err, msg, stack) {
	logger.log('Caught exception: ' + err, {color: "red"});
    ...
    initCluster: function (config) {
        ...
        cluster.on("exit", function (worker, code, signal) {
            if (code !== 0 && worker.suicide !== true) {
                logger.log("Something go wrong and a worker crashed! (" + code + ", " + signal + ")", {color: "yellow"});
                cluster.fork({config: JSON.stringify(config), session: session});
            }
        });
        
        if (config.debug) {
            logger.log("Debug enabled", {color: "grey"});
        ...
        for (i = 0; i < numberOfWorkers; i++) {
            ...
            logger.log("Initializing worker " + i, {color: "grey"});
            cluster.fork({config: JSON.stringify(config), session: session});
        }
    },
